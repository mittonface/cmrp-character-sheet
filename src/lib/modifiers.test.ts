...
describe('removeBySource', () => {
  const mods: Modifier[] = [
    {
      source: 'situation:knight',
      target: 'valour',
      operation: 'add',
      value: 1,
    },
    {
      source: 'situation:wizard',
      target: 'sorcery',
      operation: 'add',
      value: 2,
    },
    { source: 'item:sword', target: 'valour', operation: 'add', value: 3 },
    {
      source: 'accoutrement:valour:shield',
      target: 'valour',
      operation: 'add',
      value: 1,
    },
  ...
  it('removes by specific accoutrement prefix', () => {
    ...
    expect(result).toHaveLength(3);
    expect(
      result.find((m) => m.source.startsWith('accoutrement:valour:')),
    ).toBeUndefined();
  });
});
