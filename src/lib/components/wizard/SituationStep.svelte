<script lang="ts">
	...
	const sortedSituations = [...SITUATIONS].sort((a, b) => a.label.localeCompare(b.label));

	let selected = $derived(SITUATIONS.find((s) => s.id === character.situationId));
...
<div class="mx-auto max-w-4xl px-4">
	...
	<div class="mb-10 text-center">
		...
		<div class="mx-auto mt-4 flex items-center justify-center gap-3">
			...
			<span class="font-heading text-sm tracking-widest text-gold/60">I. Choose Your Situation</span>
			<span class="h-px flex-1 max-w-16 bg-gold-dark/40"></span>
		</div>
	</div>
	...
	<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
		{#each sortedSituations as situation (situation.id)}
			...
			>
				<!-- Selected indicator -->
				{#if isSelected}
					<div class="absolute -top-px -right-px rounded-bl-sm rounded-tr-sm bg-gold px-2.5 py-0.5">
						<span class="font-heading text-xs font-semibold tracking-wider text-realm uppercase">
							Selected
						</span>
					</div>
				{/if}
				...
				<!-- Situation Name -->
				<h3 class="font-heading mb-2 text-xl font-semibold text-crimson {isSelected ? 'text-crimson-dark' : ''}">
					{situation.label}
				</h3>
				...
				<div class="flex flex-wrap gap-1">
					{#each situation.requiredTraits as traitId}
						<span class="rounded-sm bg-ink/6 px-1.5 py-0.5 text-xs text-ink-light">
							{TRAIT_MAP.get(traitId)?.label}
						</span>
					{/each}
					{#each situation.requiredRetainers as retainerId}
						<span class="rounded-sm bg-gold/10 px-1.5 py-0.5 text-xs text-gold-dark">
							{RETAINER_MAP.get(retainerId)?.label}
						</span>
					{/each}
					{#if 5 - situation.requiredTraits.length - situation.requiredRetainers.length > 0}
						<span class="rounded-sm bg-parchment-300/30 px-1.5 py-0.5 text-xs text-ink-faint">
							+{5 - situation.requiredTraits.length - situation.requiredRetainers.length} more
						</span>
					{/if}
				</div>
			</button>
	...
	{#if selected}
		<div class="parchment-bg mx-auto mt-8 max-w-2xl rounded-sm border-2 border-gold/40 p-6">
