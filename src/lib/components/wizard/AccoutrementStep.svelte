<script lang="ts">
  ...
  function groupByTrait(
    accs: AccoutrementDef[],
  ): { traitId: string; traitLabel: string; items: AccoutrementDef[] }[] {
  ...
  function toggleRetainerAccoutrement(
    sid: string,
    accId: string,
    maxSlots: number,
  ) {
  ...
  function randomizeSlot(slot: CharacterSlot) {
    ...
    if (isRetainer) {
      ...
      const available = getRetainerAvailableAccoutrements(
        def.accoutrementTypes,
        character.hasRetainer,
      );
      ...
      if (primary.grantsExtra) {
        const extraOptions = getExtraAccoutrementOptions(
          primary.id,
          character.hasRetainer,
        );
...
<div class="mx-auto max-w-4xl px-4">
  ...
  <!-- Flavour intro -->
  <p
    class="mx-auto mb-10 max-w-lg text-center leading-relaxed text-parchment-300"
  >
    Each slot in your ledger comes with one <span
      class="font-heading font-semibold text-gold">accoutrement</span
    >
    — a piece of equipment, companion, or curiosity that modifies your rolls. Roll
    the dice to determine what fate provides, or choose manually below.
  </p>
    ...
    <div class="flex items-center justify-between text-sm">
      ...
      {#if !allFilled}
        <span class="font-heading text-xs tracking-wide text-gold/50">
          {totalPickable -
            (filledCount - (character.slots.length - totalPickable))} remaining
        </span>
      {/if}
    </div>
  ...
  <div class="mx-auto max-w-2xl space-y-4">
    {#each character.slots as slot, i}
      ...
      {@const accIds = character.accoutrements[sid] ?? []}
      {@const isRetainer = slot.type === 'retainer'}
      {@const retainerDef = isRetainer
        ? RETAINER_MAP.get(slot.retainerId)
        : undefined}
        ...
        <div class="flex w-full items-stretch">
          ...
          >
            ...
            <div class="min-h-[3.5rem] min-w-0 flex-1 px-4 py-2.5">
              <div class="flex items-center gap-2">
                ...
                <span
                  class="font-heading shrink-0 rounded-sm px-2 py-0.5 text-xs font-medium
										{isRetainer ? 'bg-gold/15 text-gold-dark' : 'bg-crimson/10 text-crimson'}"
                >
                ...
                <!-- Slot name -->
                <span
                  class="font-heading min-w-0 truncate text-sm font-semibold tracking-wide text-ink"
                >
                  {label}
                </span>
              ...
              {#if !canCarry}
                <span
                  class="font-heading mt-1 block text-xs tracking-wide text-ink-faint/40"
                >
              ...
              {:else if hasPick}
                <div class="mt-1 flex flex-wrap items-center gap-1.5">
                  {#each accIds as accId, ai}
                    ...
                    {/if}
                    <span
                      class="font-heading inline-block rounded-sm bg-gold/10 px-1.5 py-0.5 text-xs tracking-wide text-gold-dark/80"
                    >
              ...
              {:else}
                <span
                  class="font-heading mt-1 block text-xs tracking-wide text-crimson/50"
                >
                  Not yet outfitted
                </span>
              {/if}
        ...
        {#if isExpanded && canCarry}
          <div class="border-t border-parchment-300/30 px-4 py-5 sm:px-6">
            {#if slot.type === 'trait'}
              ...
              {@const available = getAvailableAccoutrements(
                sid,
                character.hasRetainer,
              )}
              {@const primaryId = accIds[0]}
              {@const primary = primaryId
                ? ACCOUTREMENT_MAP.get(primaryId)
                : undefined}
              ...
              {#if available.length > 0}
                ...
                <div class="grid gap-2 sm:grid-cols-2">
                  {#each available as acc (acc.id)}
                    ...
                    >
                      <span class="flex items-center gap-1.5">
                        ...
                        {#if acc.specialEffects}
                          <span class="tooltip-wrap relative">
                            <svg
                              class="h-3.5 w-3.5 shrink-0 text-crimson/50"
                              viewBox="0 0 24 24"
                              fill="currentColor"
                            >
                              <path
                        ...
                        <div class="mt-1.5 flex flex-wrap gap-1">
                          {#each acc.modifiers as mod}
                            ...
                            >
                              ...
                              {TRAIT_MAP.get(mod.target)?.label ?? mod.target}
                            </span>
                          {/each}
                          {#if acc.conditionalModifiers}
                            {#each acc.conditionalModifiers as cond}
                              <span
                                class="rounded bg-green-800/20 px-1.5 py-0.5 text-xs font-medium text-green-700"
                              >
                                {cond.description}
                              </span>
                            {/each}
                      ...
                      {#if acc.pointy}
                        <span
                          class="mt-1 inline-block text-xs text-ink-faint/40"
                          >Pointy</span
                        >
                      {/if}
                      ...
                      {#if acc.requires?.retainer}
                        <span
                          class="mt-1 inline-block text-xs text-gold-dark/50"
                          >Requires retainer</span
                        >
                      {/if}

                      {#if acc.grantsExtra}
                        <span
                          class="mt-1.5 inline-flex items-center gap-1 text-xs font-medium text-gold-dark/70"
                        >
                          <svg
                            class="h-3 w-3"
                            viewBox="0 0 24 24"
                ...
                {#if primary?.grantsExtra}
                  ...
                  <div
                    class="mt-4 rounded-sm border-2 border-dashed border-gold/30 bg-gold/5 p-4"
                  >
                    <label
                    ...
                    >
                      Bonus Accoutrement
                      <span
                        class="font-body text-xs font-normal normal-case tracking-normal text-gold-dark/60"
                      >
                          ...
                          {#if acc.modifiers.length > 0}
                            <div class="mt-1 flex flex-wrap gap-1">
                              {#each acc.modifiers as mod}
                                ...
                                >
                                  {formatModifier(mod.value)}
                                  {TRAIT_MAP.get(mod.target)?.label ??
                                    mod.target}
                                </span>
                              {/each}
                            </div>
                ...
                {/if}
              {:else}
                <p class="text-sm text-ink-faint/50">
                  No accoutrements available for this trait.
                </p>
              {/if}
            {:else}
              ...
              {#if retainerDef?.accoutrementTypes}
                ...
                {#if available.length > 0}
                  ...
                  <p
                    class="font-heading mb-3 text-xs font-medium tracking-widest text-ink-faint/70 uppercase"
                  >
                    Choose {retainerSlotCount} accoutrement{retainerSlotCount >
                    1
                      ? 's'
                      : ''}
                    <span
                      class="font-body normal-case tracking-normal text-ink-faint/50"
                    >
                    ...
                    {#each groups as group (group.traitId)}
                      <div>
                        <div class="mb-1.5 flex items-center gap-2">
                          <span
                            class="font-heading text-xs font-medium tracking-wide text-crimson/60"
                          >
                            {group.traitLabel}
                          </span>
                ...
                {:else}
                  <p class="text-sm text-ink-faint/50">
                    No accoutrements available for this retainer.
                  </p>
                {/if}
              {/if}
            {/if}
  ...
  <div class="mx-auto mt-10 max-w-lg text-center">
    ...
    {:else}
      <p class="mb-4 text-sm text-parchment-300/50">
        You may continue without outfitting every slot, but each slot is
        entitled to one free pick.
      </p>
    {/if}
    <button
