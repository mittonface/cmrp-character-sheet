...
<nav
  class="mx-auto flex max-w-xl flex-wrap items-center justify-center gap-y-1 px-2 pt-4 pb-1 sm:flex-nowrap sm:px-4"
>
...
{#if showConfirm}
  ...
  <div
    ...
    onkeydown={(e) => {
      if (e.key === 'Escape') cancelStartOver();
    }}
  >
    <div
      class="parchment-bg mx-4 max-w-sm rounded-sm border-2 border-crimson/40 p-6 shadow-[0_0_40px_rgba(0,0,0,0.5)]"
    >
      <h3 class="font-heading mb-2 text-lg font-semibold text-crimson">
        Start Over?
      </h3>
      <p class="mb-5 text-sm leading-relaxed text-ink-light">
        This will clear all your character choices and return to the beginning.
