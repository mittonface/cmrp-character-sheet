		...
		<div class="mx-auto mb-10 max-w-lg">
			{#each character.situationChoices as choice}
				...
				<div
					...
					{selectedId ? 'border-gold/50' : 'border-crimson/40 shadow-[0_0_16px_rgba(122,26,26,0.15)]'}"
				>
					<label
					...
					<div class="grid gap-2 sm:grid-cols-3">
						{#each choice.options as option}
							...
							>
								<span
									class="font-heading text-sm font-semibold {isChosen ? 'text-gold-dark' : 'text-ink-light group-hover:text-ink'}"
								>
									{option.label}
								</span>
			...
			{#each { length: 5 } as _, i}
				...
				>
					...
					<div class="flex min-h-[3.5rem] flex-1 items-center gap-3 px-4 py-2.5">
						...
						{:else}
							<!-- Empty slot -->
							<span class="font-heading text-sm italic tracking-wide text-parchment-300/50">
								{character.remainingChoices > 0 ? 'Awaiting your choice...' : '—'}
							</span>
						{/if}
					</div>
				</div>
	...
	{#if character.remainingChoices > 0}
		<div class="mx-auto mt-10 max-w-2xl">
			<div class="parchment-bg rounded-sm border-2 border-parchment-300/50 p-6">
				...
				>
					Choose {character.remainingChoices === 1 ? 'Your Final Slot' : `${character.remainingChoices} More Slots`}
				</h3>
					...
					<div>
						...
						<div class="flex flex-wrap justify-center gap-2">
							{#each character.pickableRetainers as retainer (retainer.id)}
								...
								>
									<span
										class="font-heading font-medium tracking-wide {isHovered ? 'text-gold-dark' : 'text-gold-dark/80'}"
									>
										{retainer.label}
									</span>
									<span class="ml-1 text-xs text-ink-faint/60">
										({retainer.accoutrementSlots} acc. slot{retainer.accoutrementSlots !== 1 ? 's' : ''})
									</span>
								</button>
							{/each}
						</div>
