<script lang="ts">
	...
	const CLASS_META: Record<SocialClass, { title: string; motto: string; sigil: string }> = {
		upper: {
			title: 'Upper Class',
...
<div class="mx-auto max-w-4xl px-4">
	...
	<!-- Class Cards -->
	<div class="mx-auto grid max-w-3xl gap-5 {singleClass ? 'max-w-md' : 'sm:grid-cols-' + character.availableClasses.length}">
		{#each character.availableClasses as cls (cls)}
			...
			>
				...
				<!-- Sigil -->
				<div class="mb-3 text-center text-4xl leading-none
					{isSelected ? 'text-gold' : 'text-ink/20 group-hover:text-ink/40'} transition-colors duration-300">
					{meta.sigil}
				</div>

				<!-- Class Name -->
				<h3 class="font-heading mb-1 text-center text-2xl font-bold tracking-wide
				...
				{#if extraTraits.length > 0}
					<div class="border-t border-parchment-300/50 pt-3">
						<p class="font-heading mb-1.5 text-center text-xs font-medium tracking-wide text-ink-faint uppercase">
							Required Traits
						</p>
						<div class="flex flex-wrap justify-center gap-1.5">
							{#each extraTraits as traitId}
								<span class="rounded-sm bg-ink/6 px-2 py-0.5 text-xs text-ink-light">
