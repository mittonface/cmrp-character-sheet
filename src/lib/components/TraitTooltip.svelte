<script lang="ts">
	...
	let tooltipStyle = $derived.by(() => {
		...
		const offset = left - (triggerRect.left + triggerRect.width / 2 - tooltipRect.width / 2);
...
<style>
	...
	.trait-tooltip {
		...
		background:
			radial-gradient(ellipse at 30% 20%, rgba(210, 180, 120, 0.25) 0%, transparent 50%),
			radial-gradient(ellipse at 70% 80%, rgba(200, 170, 110, 0.15) 0%, transparent 50%),
			linear-gradient(
				175deg,
