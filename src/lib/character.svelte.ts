...
export function createCharacter(initial?: CharacterData) {
  ...
  let indifferentTraitSelections = $state<string[]>(
    initial?.indifferentTraits ?? [],
  );
  let accoutrements = $state<Record<string, string[]>>(
    initial?.accoutrements ?? {},
  );
  let currencies = $state<Partial<Record<Currency, number>>>(
    initial?.currencies ?? {},
  );
  let choiceSelections = $state<Record<string, string>>(
  ...
  let freeSlotCount = $derived(
    SLOT_COUNT - slots.filter((s) => s.required).length,
  );
  ...
  let pickableIndifferentTraits = $derived.by(() => {
    ...
    return situation.availableTraits
      .filter(
        (id) =>
          !chosenTraitIds.has(id) &&
          !excluded.has(id) &&
          !alreadySelected.has(id),
      )
  ...
  let startingCurrency = $derived(
    SITUATION_MAP.get(situationId)?.startingCurrency ?? null,
  );
  ...
  function removeIndifferentTrait(traitId: string) {
    indifferentTraitSelections = indifferentTraitSelections.filter(
      (id) => id !== traitId,
    );
  }
  ...
  function addSlot(slot: CharacterSlot) {
    ...
    if (
      slot.type === 'trait' &&
      indifferentTraitSelections.includes(slot.traitId)
    ) {
      indifferentTraitSelections = indifferentTraitSelections.filter(
        (id) => id !== slot.traitId,
      );
  ...
  function setAccoutrement(
    slotId: string,
    accoutrementId: string,
    index: number = 0,
  ) {
