...
describe('data integrity', () => {
  ...
  it('all animal husbandry accoutrements have +1 animal_husbandry as first modifier', () => {
    const ahAccs = ALL_ACCOUTREMENTS.filter(
      (a) => a.slotId === 'animal_husbandry',
    );
    expect(ahAccs.length).toBe(10);
    for (const acc of ahAccs) {
      expect(acc.modifiers[0]).toEqual({
        target: 'animal_husbandry',
        value: 1,
      });
    }
  });
...
describe('specific accoutrements', () => {
  ...
  it('Burlington Wallbanger has special effects', () => {
    const bw = ACCOUTREMENT_MAP.get('burlington_wallbanger')!;
    expect(bw.specialEffects).toContain(
      'One-time use: bring down a wall of any size',
    );
  });
  ...
  it('Polearm has conditional modifiers', () => {
    ...
    expect(polearm.conditionalModifiers![0].description).toContain(
      'Lower-class',
    );
  });
  ...
  it('Cloth Sack grants extra accoutrement slot', () => {
    ...
    expect(sack.grantsExtra).toEqual({
      fromAnySlot: true,
      excludePointy: true,
    });
  });
  ...
  it('all argumentation accoutrements have +1 argumentation as first modifier', () => {
    const argAccs = ALL_ACCOUTREMENTS.filter(
      (a) => a.slotId === 'argumentation',
    );
  ...
  it('Letter of the Marque has conditional modifier vs pirates and -1 decorum', () => {
    ...
    expect(letter.modifiers.find((m) => m.target === 'decorum')!.value).toBe(
      -1,
    );
  });
  ...
  it('A Box grants extra accoutrement and has setup special effect', () => {
    ...
    expect(box.grantsExtra).toEqual({
      fromAnySlot: true,
      excludePointy: false,
    });
  ...
  it('Removable Moustache has conditional modifier vs French persons and -1 chastity', () => {
    ...
    expect(
      moustache.modifiers.find((m) => m.target === 'chastity')!.value,
    ).toBe(-1);
  });
  ...
  it('Punch and Judy Puppets has conditional modifier vs Lower-Class and -1 authority', () => {
    ...
    expect(puppets.conditionalModifiers![0].description).toContain(
      'Lower-Class',
    );
    expect(puppets.modifiers.find((m) => m.target === 'authority')!.value).toBe(
      -1,
    );
  });
  ...
  it('Sir Dagonet Mask has conditional modifier vs Knights of the Round Table', () => {
    ...
    expect(mask.conditionalModifiers![0].description).toContain(
      'Knights of the Round Table',
    );
  });

  it('all heartiness accoutrements have +1 heartiness as first modifier', () => {
    const heartAccs = ALL_ACCOUTREMENTS.filter(
      (a) => a.slotId === 'heartiness',
    );
  ...
  it('Wooden Churn of Bodily Substance has conditional modifier vs Gumbys and -1 wisdom', () => {
    ...
    expect(
      churn.modifiers.find((m) => m.target === 'wisdom_in_the_ways_of_science')!
        .value,
    ).toBe(-1);
  });
  ...
  it('Surgical Stockings has +1 chastity and -1 decorum', () => {
    ...
    expect(
      stockings.modifiers.find((m) => m.target === 'chastity')!.value,
    ).toBe(1);
    expect(stockings.modifiers.find((m) => m.target === 'decorum')!.value).toBe(
      -1,
    );
  });
  ...
  it('all lorefulness accoutrements have +1 lorefulness as first modifier', () => {
    const loreAccs = ALL_ACCOUTREMENTS.filter(
      (a) => a.slotId === 'lorefulness',
    );
  ...
  it('A Short History of History Books has conditional modifier vs scholars and -1 argumentation', () => {
    ...
    expect(
      book.modifiers.find((m) => m.target === 'argumentation')!.value,
    ).toBe(-1);
  });
  ...
  it('Short History of Chairs has conditional modifier vs Burghers and -1 glibness', () => {
    ...
    expect(chairs.modifiers.find((m) => m.target === 'glibness')!.value).toBe(
      -1,
    );
  });

  it('World Encyclopaedia of Carnal Knowledge has +1 wisdom, -1 chastity, and special effects', () => {
    const enc = ACCOUTREMENT_MAP.get(
      'world_encyclopaedia_of_carnal_knowledge',
    )!;
    expect(
      enc.modifiers.find((m) => m.target === 'wisdom_in_the_ways_of_science')!
        .value,
    ).toBe(1);
  ...
  it("Rabbit's Foot has +2 luck total and -1 animal husbandry", () => {
    ...
    expect(
      foot.modifiers.find((m) => m.target === 'animal_husbandry')!.value,
    ).toBe(-1);
  });
  ...
  it('Pagan Goddess Figurine has conditional modifier vs Barbarians and -1 purpose', () => {
    ...
    expect(figurine.conditionalModifiers![0].description).toContain(
      'Barbarians',
    );
    expect(figurine.modifiers.find((m) => m.target === 'purpose')!.value).toBe(
      -1,
    );
  });
  ...
  it('Garden Gnome has conditional modifier vs Vikings and -1 authority', () => {
    ...
    expect(gnome.modifiers.find((m) => m.target === 'authority')!.value).toBe(
      -1,
    );
  });
  ...
  it("Deer Antler Hat has conditional modifier vs Knights Who Say 'Ni' and -1 subtlety", () => {
    ...
    expect(hat.conditionalModifiers).toHaveLength(1);
    expect(hat.conditionalModifiers![0].description).toContain(
      "Knights Who Say 'Ni'",
    );
  ...
  it('Llama-Skin Gloves has +1 glibness and llamas attack', () => {
    ...
    expect(gloves.modifiers.find((m) => m.target === 'glibness')!.value).toBe(
      1,
    );
  ...
  it('Pouch of Rid-a-Weasel has conditional modifier vs woodland creatures, -1 heartiness, and one-time use', () => {
    ...
    expect(pouch.conditionalModifiers![0].description).toContain(
      'small woodland creatures',
    );
    expect(pouch.modifiers.find((m) => m.target === 'heartiness')!.value).toBe(
      -1,
    );
  ...
  it('Pate Plank has conditional modifier vs clergy, -1 wisdom, and one-time use', () => {
    ...
    expect(
      plank.modifiers.find((m) => m.target === 'wisdom_in_the_ways_of_science')!
        .value,
    ).toBe(-1);
  ...
  it('Holy Hand Grenade of Antioch has +1 strategy, -1 valour, and one-time use', () => {
    ...
    expect(grenade.modifiers.find((m) => m.target === 'strategy')!.value).toBe(
      1,
    );
    expect(grenade.modifiers.find((m) => m.target === 'valour')!.value).toBe(
      -1,
    );
  ...
  it('Cauldron has conditional modifier vs Witches and cooks and -1 nimbleness', () => {
    ...
    expect(
      cauldron.modifiers.find((m) => m.target === 'nimbleness')!.value,
    ).toBe(-1);
  });
  ...
  it('Fetchez la Vache Set has conditional modifier and -1 animal husbandry', () => {
    ...
    expect(set.conditionalModifiers![0].description).toContain(
      'Fetchez la Vache',
    );
    expect(
      set.modifiers.find((m) => m.target === 'animal_husbandry')!.value,
    ).toBe(-1);
  });
  ...
  it('Crossbow has conditional modifier vs armoured persons and -1 nimbleness', () => {
    ...
    expect(xbow.modifiers.find((m) => m.target === 'nimbleness')!.value).toBe(
      -1,
    );
  });
  ...
  it('Snare Trap has conditional modifier vs Beasts and -1 animal husbandry', () => {
    ...
    expect(
      trap.modifiers.find((m) => m.target === 'animal_husbandry')!.value,
    ).toBe(-1);
  });
  ...
  it('all wisdom_in_the_ways_of_science accoutrements have +1 wisdom_in_the_ways_of_science as first modifier', () => {
    const wisAccs = ALL_ACCOUTREMENTS.filter(
      (a) => a.slotId === 'wisdom_in_the_ways_of_science',
    );
    expect(wisAccs.length).toBe(10);
    for (const acc of wisAccs) {
      expect(acc.modifiers[0]).toEqual({
        target: 'wisdom_in_the_ways_of_science',
        value: 1,
      });
    }
  });
  ...
  it('Abacus set has conditional modifier vs Shoppekeepers and -1 glibness', () => {
    ...
    expect(abacus.conditionalModifiers![0].description).toContain(
      'Shoppekeepers',
    );
    expect(abacus.modifiers.find((m) => m.target === 'glibness')!.value).toBe(
      -1,
    );
  });
  ...
  it('Astrolabe has conditional modifier vs forward-thinkers and -1 purpose', () => {
    ...
    expect(astrolabe.conditionalModifiers![0].description).toContain(
      'forward-thinkers',
    );
    expect(astrolabe.modifiers.find((m) => m.target === 'purpose')!.value).toBe(
      -1,
    );
  });
...
describe('getAvailableAccoutrements', () => {
  ...
  it('excludes retainer-required accoutrements when player has no retainer', () => {
    ...
    expect(
      available.find((a) => a.id === 'burlington_wallbanger'),
    ).toBeUndefined();
  });
