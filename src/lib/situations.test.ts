...
describe('data integrity', () => {
  ...
  it('situation required + available slots do not exceed SLOT_COUNT', () => {
    for (const sit of SITUATIONS) {
      const baseRequired =
        sit.requiredTraits.length + sit.requiredRetainers.length;
...
describe('Knight situation', () => {
  ...
  it('is indifferent to subtlety', () => {
    expect(knight.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['subtlety'],
    });
  });
  ...
  it('starts with gold (1d30)', () => {
    expect(knight.startingCurrency).toEqual({
      currency: 'gold',
...
describe('Churl situation', () => {
  ...
  it('allows selecting 2 indifferent traits excluding luck', () => {
    expect(churl.indifferentTraits).toEqual({
      type: 'select',
      count: 2,
      exclude: ['luck'],
    });
...
describe('Eremite situation', () => {
  ...
  it('is indifferent to decorum', () => {
    expect(eremite.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['decorum'],
    });
  });
...
describe('Cleric situation', () => {
  ...
  it('has 4 available traits to choose from', () => {
    ...
    expect(cleric.availableTraits).toEqual([
      'argumentation',
      'glibness',
      'luck',
      'valour',
...
describe('Knave situation', () => {
  ...
  it('requires subtlety, glibness, and nimbleness', () => {
    expect(knave.requiredTraits).toEqual([
      'subtlety',
      'glibness',
      'nimbleness',
    ]);
  ...
  it('is indifferent to valour', () => {
    expect(knave.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['valour'],
    });
  });
...
describe('Monarch situation', () => {
  ...
  it('requires authority, purpose, and strategy', () => {
    expect(monarch.requiredTraits).toEqual([
      'authority',
      'purpose',
      'strategy',
    ]);
  ...
  it('is indifferent to argumentation', () => {
    expect(monarch.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['argumentation'],
    });
  });
...
describe('Monk/Nun situation', () => {
  ...
  it('is indifferent to glibness', () => {
    expect(monkNun.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['glibness'],
    });
  });
...
describe('Noble situation', () => {
  ...
  it('is indifferent to druidry', () => {
    expect(noble.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['druidry'],
    });
  });
...
describe('Enchanter situation', () => {
  ...
  it('is indifferent to purpose', () => {
    expect(enchanter.indifferentTraits).toEqual({
      type: 'fixed',
      traitIds: ['purpose'],
    });
  });
...
describe('getRequiredSlots', () => {
  it('returns required trait and retainer slots for knight', () => {
    ...
    expect(traitSlots[0]).toEqual({
      type: 'trait',
      traitId: 'valour',
      required: true,
    });
    expect(traitSlots[1]).toEqual({
      type: 'trait',
      traitId: 'chastity',
      required: true,
    });
  ...
  it('includes decorum for cleric with upper class', () => {
    ...
    const traitIds = slots
      .filter((s) => s.type === 'trait')
      .map((s) => s.traitId);
