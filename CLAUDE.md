...
- **Accoutrements** — equipment/items attached to a Trait or Retainer. Every required slot (both traits and retainers) gets one free accoutrement pick at character creation; additional accoutrements can be acquired during play for any slot. Accoutrements are defined **per-slot** (not per-situation) — any situation with Valour uses the same Valour accoutrement list; retainer accoutrements are keyed by retainer ID the same way. `AccoutrementDef.slotId` holds the trait or retainer ID it belongs to. Lookups use `ACCOUTREMENTS_BY_SLOT` (keyed by slot ID) and `getAvailableAccoutrements(slotId, hasRetainer)`. They provide simple modifiers (+/- to trait rolls), conditional modifiers (display-only text like "+1 vs Monstrosities"), and special effects. Some have prerequisites (e.g., requires a retainer). **Important:** accoutrement modifiers are **roll modifiers**, not die-size modifiers. A +1 to Valour means the roll is 1d12+1, not 1d13. They are tracked via `rollModifiers` (a derived `Record<string, number>` in character state), completely separate from the main `modifiers` stack which affects die sizes.
