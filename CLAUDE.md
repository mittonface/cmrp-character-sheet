...
- A Situation defines: required traits, available traits to pick from, required retainers, whether retainers are allowed, which retainer types are available, and optionally class-required traits (`classRequiredTraits`) that add required trait slots based on social class choice.
