...
- **Wizard UI** — character creation is a multi-step wizard. Step components live in `src/lib/components/wizard/` and receive the `character` state object as a prop. The wizard orchestration (step routing, navigation) lives in `+page.svelte`. Steps: I. Situation → II. Class → III. Slots (trait/retainer selection + situation choices like Muse + indifferent trait rolling for Churl) → IV. Accoutrements (roll or manually pick one free accoutrement per slot — "Roll All" randomly assigns accoutrements to all slots, per-slot dice buttons roll individually with a cycling animation; traits get a primary pick with optional bonus from `grantsExtra`, retainers pick up to `accoutrementSlots` from their allowed trait pools) → V. Details (rest of sheet, will be broken up further). `WizardBreadcrumbs` provides top-of-page navigation with Roman numeral labels; past steps are clickable, future steps are disabled. A "Start Over" button clears all state with a confirmation dialog. Each step component receives `{ character, onadvance }` props — mutates character state directly and calls `onadvance()` to proceed.
...
- **Shared UI components** — `src/lib/components/TraitTooltip.svelte` provides a parchment-styled hover tooltip for trait descriptions (wraps any child content, takes a `description` string prop).
...
- **Indifferent Traits** — each Situation specifies traits the character is indifferent to, via `IndifferentTraitsDef`. Two variants: `{ type: 'fixed', traitIds: [...] }` for Situations with predetermined indifferent traits (e.g., Knight is always indifferent to Subtlety), and `{ type: 'select', count: N, exclude?: [...] }` for Situations where the player randomly rolls N indifferent traits from their unchosen available traits (e.g., Churl rolls 2, excluding Luck). For `select` type, a "Roll" button in the SlotsStep randomly picks traits with a cycling dice animation (same pattern as accoutrement rolling). Selections are only available once all 5 slots are filled; the Continue button is disabled until indifferent traits are complete.
